...
public class Path_AStar {
	...
	Queue<Tile> path;
	Tile destination;
	public Path_AStar(Queue<Tile> p, Tile dest) {
		path = p;
		destination = dest;
	}
	public Path_AStar(Building building, Tile tileStart, Tile tileEnd) {

		destination = tileEnd;
		// Check to see if we have a valid tile graph
		if(building.pathTileGraph == null) {
			building.pathTileGraph = new Path_TileGraph(building);
		}

		// A dictionary of all valid, walkable nodes.
		Dictionary<Tile, Path_Node<Tile>> nodes = building.pathTileGraph.nodes;
...
*/
	...
	float heuristic_cost_estimate( Path_Node<Tile> a, Path_Node<Tile> b ) {
		...
		return Mathf.Sqrt(
			Mathf.Pow(a.data.getX() - b.data.getX(), 2) +
			Mathf.Pow(a.data.getY() - b.data.getY(), 2)
		);
	...
	float dist_between( Path_Node<Tile> a, Path_Node<Tile> b ) {
		...
		if( Mathf.Abs( a.data.getX() - b.data.getX() ) + Mathf.Abs( a.data.getY() - b.data.getY() ) == 1 ) {
			return 1f;
		}

		// Diag neighbours have a distance of 1.41421356237	
		if( Mathf.Abs( a.data.getX() - b.data.getX() ) == 1 && Mathf.Abs( a.data.getY() - b.data.getY() ) == 1 ) {
